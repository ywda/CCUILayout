...
#import "CCUIContainerSuperBaseView.h"


@interface CCUIContainerSuperBaseView()

@property (nonatomic, assign) CCUIContainerViewState state;
@property (nonatomic, strong) UIView *movieViewParentView;
@property (nonatomic) CGRect movieViewFrame;

@end

@implementation CCUIContainerSuperBaseView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.state = CCUIContainerViewStateSmall;
    }
    return self;
}
...
- (void) enterFullscreen{
    ...
    if (self.state != CCUIContainerViewStateSmall) {
        return;
    }

    self.state = CCUIContainerViewStateAnimating;
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.state = CCUIContainerViewStateFullscreen;
    }];
}
...
- (void) exitFullscreen{
    ...
    if (self.state != CCUIContainerViewStateFullscreen) {
        return;
    }
...
- (void) autoEnterOrExitFullscreen
{
    if (self.state != CCUIContainerViewStateSmall) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.35 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
           !self.chUiLayoutRb?:self.chUiLayoutRb(CCUIContainerViewStateSmall);
        });
    ...
    if (self.state != CCUIContainerViewStateFullscreen) {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            !self.chUiLayoutRb?:self.chUiLayoutRb(CCUIContainerViewStateFullscreen);
        }];
