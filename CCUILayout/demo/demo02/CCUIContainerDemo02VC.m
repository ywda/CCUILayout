//
//  CCUIContainerDemo02VC.m
//  CCUIContainer
//  MARK: 模拟从plist快速创建 页面 demo 父类 CCUIContainerBaseVC
//  Created by Admin on 2020/4/30.
//  Copyright © 2020 Admin. All rights reserved.
//

#import "CCUIContainerDemo02VC.h"
#import "CCUIContainer_Func.h"
...
@interface CCUIContainerDemo02VC ()

@end

@implementation CCUIContainerDemo02VC
...
- (void) simulation_SubViews
{
    ...
    [self wholeUIControls:^(NSArray<UIResponder *> * subviews, NSMutableArray<CCUIContainerUiMode *> * cc_subviewModes) {
        ...
        weakSelf.cc_subviews = subviews;
        weakSelf.cc_subviewModes = [NSMutableArray arrayWithArray:cc_subviewModes];
...
- (void) simulation_Update_SubViews
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        // 模拟改变 subview 的高度
        for (int i = 0; i < weakSelf.cc_subviewModes.count; i++) {
            self.cc_subviewModes[i].height += (i==0) ? 100 : 0;
        }
        
        [self updateUIControls:self.cc_subviewModes animation:YES];
    });
}
...
- (void) actionReuseUiControl_For_WDZMyInfoSegLine: (WDZMyInfoSegLine*)rp
{
    switch ([rp.CLM.bind intValue]) {
        case -1:
            {
                rp.backgroundColor = CCUIContainer_RED;
                
            }
            break;
        case -2:
            {
                rp.backgroundColor = CCUIContainer_GREEN;
            }
            break;
        case -3:
            {
                rp.backgroundColor = CCUIContainer_ORANG;
            }
            break;
        case -4:
            {
                rp.backgroundColor = CCUIContainer_YELLOW;
            }
            break;
        default:
        {
            rp.backgroundColor = CCUIContainer_BLACK;
        }
            break;
    }
}
...
- (void) actionReuseUiControl_For_WDZMyInfoItem00: (WDZMyInfoItem00*)rp
{
    ...
    switch ([rp.CLM.bind intValue]) {
        case 1:
            {
                rp.backgroundColor = CCUIContainer_RANDOM_COLOR;
        ...
        default:
        {
            rp.backgroundColor = CCUIContainer_WHITE;
        }
            break;
    }
}
